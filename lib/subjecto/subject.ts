...
interface Subject<T> {
  ...
  useLocalStorage?: [(value: string) => T, (value: T) => string];
  hook: (nextValue?: T) => T;
}
...
Subject.prototype.saveToLocalStorage = function () {
  ...
  if (
    typeof localStorage !== "undefined" &&
    this.useLocalStorage?.length === 2
  ) {
    try {
      localStorage.setItem(
        defaultLocalStoragePrefix + this.name,
        this.useLocalStorage[1](this.value)
      );
      return true;
    } catch (error) {
      console.log(error);
